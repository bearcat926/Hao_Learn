...
| 当前session**不能查询没有锁定的表（防止死锁）**<br />select * from person<br />Table 'person' was not locked with LOCK TABLES | 当前session可以查询或者更新未锁定的表<br />select * from mylock<br />insert into person values(1,'zhangsan'); |
| 当前session**插入或者更新表会提示错误**<br />insert into mylock values(6,'f')<br />Table 'mylock' was locked with a READ lock and can't be updated<br />update mylock set name='aa' where id = 1;<br />Table 'mylock' was locked with a READ lock and can't be updated | 当前session插入数据会等待获得锁<br />insert into mylock values(6,'f'); |
...
| 当前session**不能对表进行更新或者插入操作**<br />insert into mylock values(6,'f')<br />Table 'mylock' was locked with a READ lock and can't be updated<br />update mylock set name='aa' where id = 1;<br />Table 'mylock' was locked with a READ lock and can't be updated |    其他session可以查询该表的记录<br />select* from mylock    |
| 当前session**不能查询没有锁定的表**<br />select * from person<br />Table 'person' was not locked with LOCK TABLES | **其他session可以进行插入操作，但是更新会阻塞**<br />update mylock set name = 'aa' where id = 1; |
|        当前session**不能访问其他session插入的记录**；        |                                                              |
|                  释放锁资源：unlock tables                   |               当前session获取锁，更新操作完成                |
|  当前session**可以在释放锁资源后查看其他session插入的记录**  |                                                              |
...
​		**共享锁（S）**：又称读锁。**允许多个事务对于同一数据可以共享一把用于读取数据库的锁，而阻止其他事务获得同一数据的排他锁；不允许任何事务更新数据**

​			*理解：如果不阻止排他锁，当使用排他锁时，会导致共享锁和排他锁共同使用，这是矛盾的*

若事务T对数据对象A加上S锁，则事务T可以读A但不能修改A，其他事务只能再对A加S锁，而不能加X锁，直到T释放A上的S锁。这保证了其他事务可以读A，但在T释放A上的S锁之前不能对A做任何修改。
		**排他锁（X）**：又称写锁。**阻止其他事务取得其他锁，即不允许排他锁与其他锁共存；仅允许获取排他锁的事务更新数据**

若事务T对数据对象A加上X锁，事务T可以读A也可以修改A，其他事务不能再对A加任何锁，直到T释放A上的锁。

​	    **意向共享锁（IS）**：表示事务准备给数据行加上共享锁，也就是说一个数据行在加共享锁之前必须先取得该表的IS锁。

​		**意向排他锁（IX）**：表示事务准备给数据行加上排他锁，也就是说一个数据行在加排他锁之前必须先取得该表的IS锁。

​		**意向锁是InnoDB在操作数据之前自动加的，不需要用户干预。**

​		自增锁（innodb_autoinc_lock_mode）：针对自增列自增长的一个特殊的表级别锁，**默认为1，事务未提交则id永久丢失。**
...
由于mysql的行锁是针对索引加的锁，不是针对记录加的锁，所以对于虽然是访问不同行的记录，但是依然无法访问到具体的数据
