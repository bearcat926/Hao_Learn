...
每个线程对应一个栈，一个方法对应一块栈帧内存区域，即栈帧也叫方法帧
，其中包括局部变量表（存放局部变量），操作数栈，动态链接（运行时将方法区内对应的方法），方法出口；栈用栈帧的局部变量指向堆中对象的方式，与堆建立联系；

Java classes在Java hotspot VM内部表示为类元数据。
在之前Java hotspot VM版本，类元数据在持久代中分配，JDK8之后,持久代被移除，并且类元数据在本地内存中分配。

方法区在jDK1.8之后称作元空间（Metaspace），并且不使用运行时内存而使用直接内存，即离堆内存；方法区包括常量，静态变量和类元信息（即类的组成部分 - 属性、方法、内部类）
