...
（数尾端）途径的黑色节点个数。根据约束条件的第4、5条，可以推出对于任意高度的节点，它的黑深度都满足 Black Depth >= height / 2 。也就是说，对于任意包含 n 个节点的红黑树而言，它的根节点高度 h <= 2log~2~( n+1 ) 。常规 BST 操作比如查找、插入、删除等，时间复杂度为 *O*(*h*) ，即取决于树的高度 。当树失衡时，时间复杂度将有可能恶化到 *O*(*n*)，即h=n 。所以，当我们能保证树的高度始终保持在 *O*(log*n*）时，便能保证所有操作的时间复杂度都能保持在 *O*(log*n*） 以内。
